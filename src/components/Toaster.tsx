      ...
      {(toast) => (
        ...
        >
          <Show when={toast().type === 'success'}>
            <MdOutlineCheck_circle class={cn('shrink-0 size-5 text-green-500')} />
          </Show>
          <Show when={toast().type === 'info'}>
            <MdOutlineInfo class={cn('shrink-0 size-5 text-blue-500')} />
          </Show>
          <Show when={toast().type === 'warning'}>
            <MdOutlineWarning class={cn('shrink-0 size-5 text-amber-500')} />
          </Show>
          <Show when={toast().type === 'error'}>
            <MdOutlineError class={cn('shrink-0 size-5 text-red-500')} />
          </Show>
          <Show when={toast().type === 'loading'}>
            <MdOutlineRefresh class={cn('shrink-0 size-5 animate-spin text-muted-foreground')} />
          </Show>
