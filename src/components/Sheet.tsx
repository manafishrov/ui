...
const getPositionerClass = (side: string): CnReturn => cn(
...
const getContentClass = (side: string): CnReturn => cn(
...
export const SheetContent: Component<SheetContentProps> = (props) => {
  ...
  return (
    <Portal>
      <SheetOverlay />
      <SheetPrimitive.Positioner class={getPositionerClass(side)}>
        <SheetPrimitive.Content
          data-slot='sheet-content'
          data-side={side}
          class={cn(getContentClass(side), local.class)}
          {...others}
        >
          {local.children}
          {showCloseButton && (
            <SheetPrimitive.CloseTrigger
              ...
              asChild={(props) => (
                <Button variant='ghost' size='icon-sm' {...props}>
                  <MdOutlineClose />
                  <span class='sr-only'>{`${translator('ui.close')}`}</span>
                </Button>
              )}
            />
          )}
